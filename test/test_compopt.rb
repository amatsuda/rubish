...
class TestCompopt < Test::Unit::TestCase
  ...
  def test_compopt_disable_option
    Rubish::Builtins.run_compopt(['-o', 'nospace', 'git'])
    assert Rubish::Builtins.completion_option?('git', 'nospace')
  ...
  def test_compopt_combined_enable_form
    result = Rubish::Builtins.run_compopt(['-onospace', 'git'])
    assert_true result
    assert Rubish::Builtins.completion_option?('git', 'nospace')
  end
  ...
  def test_compopt_default_completion
    Rubish::Builtins.run_compopt(['-o', 'dirnames', '-D'])
  ...
  def test_compopt_empty_completion
    Rubish::Builtins.run_compopt(['-o', 'bashdefault', '-E'])
  ...
  def test_compopt_invalid_option_returns_false
    $stderr = StringIO.new
    result = Rubish::Builtins.run_compopt(['-o', 'invalid_option', 'git'])
    assert_false result
  end

  def test_compopt_invalid_option_prints_error
    output = capture_stderr do
      Rubish::Builtins.run_compopt(['-o', 'badopt', 'git'])
    end
  ...
  def test_compopt_invalid_disable_option_returns_false
    $stderr = StringIO.new
    result = Rubish::Builtins.run_compopt(['+o', 'invalid', 'git'])
    assert_false result
  end
  ...
  def test_compopt_all_valid_options
    ...
    valid_options.each do |opt|
      result = Rubish::Builtins.run_compopt(['-o', opt, 'testcmd'])
  ...
  def test_compopt_print_command_options_empty
    output = capture_stdout do
      Rubish::Builtins.run_compopt(['git'])
    end
    assert_match(/git: no options/, output)
  end

  def test_compopt_print_command_options
    Rubish::Builtins.run_compopt(['-o', 'nospace', '-o', 'dirnames', 'git'])
    output = capture_stdout do
      Rubish::Builtins.run_compopt(['git'])
    end
  ...
  def test_compopt_print_default_options
    Rubish::Builtins.run_compopt(['-o', 'filenames', '-D'])
    output = capture_stdout do
      Rubish::Builtins.run_compopt(['-D'])
    end
  ...
  def test_compopt_combined_default_empty_and_commands
    Rubish::Builtins.run_compopt(['-o', 'nospace', '-D', '-E', 'git', 'svn'])
